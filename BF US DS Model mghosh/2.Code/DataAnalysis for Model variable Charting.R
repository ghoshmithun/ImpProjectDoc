...
var_names <- setdiff(colnames(data_full),c("customer_key","Response","acquisition_date","masterkey","online_last_txn_date","resp_disc_percent","offline_last_txn_date","offline_disc_last_txn_date","customerkey","addr_zip_code"))
data_full_var_types <- sapply(data_full[,var_names],class) 

data_full$card_status <- as.factor(data_full$card_status)

var_plot(var_names=var_names , Response=data_full$Response,data=data_full ,file_to_save = "//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/BRFS_Variable_Response_Plot.pdf")
