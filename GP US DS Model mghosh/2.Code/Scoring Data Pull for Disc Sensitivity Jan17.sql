 ...
 */
set database=mghosh_ds_jan17;
SET DATE_NOW=2017-01-01;
set mapred.job.queue.name=aa;
set mapred.job.priority=VERY_HIGH;
SET hive.cli.print.header=true;
SET DATEINDEX = 20170101;
...
drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year as
...
where  
(
(order_status='R' and unix_timestamp(to_date(ship_date),'yyyy-MM-dd') between UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd') AND 
 UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd'))
or 
(order_status='O' and unix_timestamp(to_date(demand_date),'yyyy-MM-dd') between UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd') AND 
 UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd'))
) 
...
--#customer-- 8703553
 ...
 --DM Campaign
  drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_DM_Campaign_Txn_tmp ;
  create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_DM_Campaign_Txn_tmp  as
  select T1.customer_key, T2.cell_key, T2.Camp_Start_Date , T2.Camp_end_date
  from (select distinct customer_key from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year)  T1 
  left outer join ( select a.customer_key, a.cell_key,b.GAP_CELL_START_DATE as Camp_Start_Date ,b.gap_cell_end_date as Camp_end_date from mds_new.ods_mailingmailhouse_t a inner join mds_new.ods_cell_t b on a.cell_key=b.cell_key where a.brand ='GP' and a.country='US' and a.event_type='M' and 
  UNIX_TIMESTAMP(b.GAP_CELL_START_DATE, 'yyyy-MM-dd_HH-mm-ss_Z') BETWEEN  UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd') AND  UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd') 
  or UNIX_TIMESTAMP(b.gap_cell_end_date, 'yyyy-MM-dd_HH-mm-ss_Z') BETWEEN  UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd')
  AND  UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd')  ) T2 on T1.customer_key=T2.customer_key  ;
  
 drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_DM_Campaign_Txn2_tmp;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_DM_Campaign_Txn2_tmp as
  ...
  from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year a left outer join ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_DM_Campaign_Txn_tmp b on a.customer_key=b.customer_key;
...
drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model1_tmp ;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model1_tmp as 
...
from (select distinct customer_key from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year) a 
left outer join 
(select customer_key,  max(transaction_date) as Offline_Last_txn_date from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year where order_status='R' 
group by customer_key) b
on a.customer_key = b.customer_key
left outer join
(select customer_key,  max(transaction_date) as Offline_disc_Last_txn_date from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year where order_status='R' 
and discount_amt < 0 group by customer_key) c
on a.customer_key = c.customer_key
left outer join
(select customer_key,  max(transaction_date) as Online_Last_txn_date from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year where order_status='O' 
group by customer_key) d
on a.customer_key = d.customer_key
left outer join
(select cust_key, min(from_unixtime(UNIX_TIMESTAMP(to_date(acquisition_date),'yyyy-MM-dd'))) as acquisition_date , sum(tot_plc_cards) as Total_PLCC_Cards from mds_new.ods_brand_customer_t  where  brand = '${hiveconf:CURRENT_BRAND}' 
and country='US' group by cust_key ) e
on a.customer_key=e.cust_key ;

select cust_key,from_unixtime(UNIX_TIMESTAMP(to_date(acquisition_date),'yyyy-MM-dd')), to_date(substr(acquisition_date,1,10),'yyyy-MM-dd'),tot_plc_cards from 
mds_new.ods_brand_customer_t  where  brand = '${hiveconf:CURRENT_BRAND}' limit 10; 
--select count(1) from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_offline_model1_tmp;
----  8728606

-- 2	Monetary	Average order size retail & Online together	6 months and 12 months

drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model2_tmp  ;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model2_tmp  as
...
from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model1_tmp a left outer join
...
from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year
...
as Num_Order_num_last_12_mth from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year
where transaction_date between DATE_ADD('${hiveconf:DATE_NOW}', - 367) AND DATE_ADD('${hiveconf:DATE_NOW}', - 1) group by customer_key) t2
on t1.customer_key=t2.customer_key ;
--8728606

-- 1	Monetary	% of discounted  item purchase in dollars	6 months and 12 months

 drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_discounted_item_tmp ;
 create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_discounted_item_tmp as 
select t2.customer_key, t1.Percent_disc_last_6_mth, t2.Percent_disc_last_12_mth from 
 ...
 from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year  where transaction_date between DATE_ADD('${hiveconf:DATE_NOW}', - 183) 
 ...
 from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year where transaction_date between DATE_ADD('${hiveconf:DATE_NOW}', -  367) AND
...
-- Non PLCC MPC promotion Codes
 drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_Monetary_disc_comm_resp_temp_2 ;
 create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_Monetary_disc_comm_resp_temp_2 as   
...
and discount_reason_cd in ('117', '521', '646', '893') and  transaction_date between DATE_ADD('${hiveconf:DATE_NOW}', - 367) AND DATE_ADD('${hiveconf:DATE_NOW}', - 1) group by customer_key ;
...
where  
(
(b.order_status='R' and unix_timestamp(b.ship_date,'yyyy-MM-dd') between UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd') AND 
 UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd'))
or 
(b.order_status='O' and unix_timestamp(to_date(b.demand_date),'yyyy-MM-dd') between UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 367), 'yyyy-MM-dd') AND 
 UNIX_TIMESTAMP(DATE_ADD('${hiveconf:DATE_NOW}', - 1), 'yyyy-MM-dd'))) 
...
drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model3_tmp  ;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model3_tmp as   
...
from  ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model2_tmp a 
left outer join 
...
on a.customer_key=b.customer_key
left outer join   ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_Monetary_disc_comm_resp_temp_2  c 
on a.customer_key=c.customer_key
left outer join
${hiveconf:database}.${hiveconf:CURRENT_BRAND}_discounted_item_tmp  d 
on a.customer_key=d.customer_key 
left outer join 
...
left outer join
( select customer_key , sum(gross_sales_amt + discount_amt) as gp_net_sales_amt_12_mth from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year group by customer_key) h 
...
from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year a left outer join (select transaction_num ,  sum(discount_amt) as discount_amt from mds_new.ods_orderline_discounts_t where brand='GP'  and country= 'US' and discount_reason_cd not in ('505','513','562','638','661')  group by transaction_num ) b
on a.transaction_num =b.transaction_num) x group by x.customer_key ;
...
(select a.customer_key, a.gross_sales_amt,a.ship_date,a.transaction_date,a.transaction_num,b.discount_amt , (coalesce(a.gross_sales_amt,cast(0 as float)) + coalesce(b.discount_amt,cast(0 as float)))  as net_sales_amt
from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year a left outer join (select transaction_num ,  sum(discount_amt) as discount_amt from mds_new.ods_orderline_discounts_t where brand='GP'  and country= 'US' and discount_reason_cd  in ('505','513','562','638','661')  group by transaction_num ) b
on a.transaction_num =b.transaction_num) x group by x.customer_key ;
...
-- 20	Ratio of communication sent through electronic medium	12 months and EM plus DM
  ...
  drop table if exists ${hiveconf:database}.Total_DM_EM_12mth;
   ...
   from    (select distinct customer_key from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year)  a left outer join (select  customer_key,count(1) as Num_DM_Campaign from mds_new.ods_mailingmailhouse_t group by customer_key ) b on a.customer_key=b.customer_key) t1 left outer join
   (select customer_key, count(1) as Num_EM_Campaign from mds_new.ods_mailingemail_t group by customer_key) t2
   on t1.customer_key=t2.customer_key;
   

drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_order_txn_size ;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_order_txn_size as 
select a.customer_key,a.Num_units_6mth,a.Num_txn_6mth,b.Num_units_12mth,b.Num_txn_12mth from
 ...
 full outer  join 
 (select y.customer_key, count(y.line_num) as Num_units_12mth, count(distinct y.transaction_num) as Num_txn_12mth from mds_new.Ods_orderline_t y where y.transaction_date between DATE_ADD('${hiveconf:DATE_NOW}', - 367) AND DATE_ADD('${hiveconf:DATE_NOW}', - 1) group by y.customer_key ) b
on a.customer_key=b.customer_key;
...
from
(select a.customer_key,a.transaction_num,b.line_num, b.product_key from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year a inner join mds_new.ods_orderline_t b on a.transaction_num=b.transaction_num ) t1 left outer join  mds_new.ods_product_t t2
...
create table ${hiveconf:database}.ticket_size_ratio as  
 ...
 (select distinct customer_key from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year)  x
 left outer join
(select customer_key, sum(coalesce(gross_sales_amt,cast(0 as float)) + coalesce(discount_amt,cast(0 as float)))/count(transaction_num) as disc_ATS from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year  where discount_amt < 0 
group by customer_key ) a 
on x.customer_key=a.customer_key
left outer join 
(select customer_key, sum(coalesce(gross_sales_amt,cast(0 as float)) + coalesce(discount_amt,cast(0 as float)))/count(transaction_num) as non_disc_ATS from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_customer_txn_last_year where discount_amt = 0 
group by customer_key ) b 
...
drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model4_tmp ;
create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model4_tmp as 
...
from  
${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model3_tmp a 
left outer join
...
on a.customer_key=d.customer_key
left outer join
${hiveconf:database}.${hiveconf:CURRENT_BRAND}_order_txn_size e
on a.customer_key=e.customer_key
left outer join
...
on a.customer_key=k.customer_key  ;
 ...
 drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model5_tmp ;
 create table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model5_tmp as
 select  t1.*, t2.*
 from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model4_tmp  t1 
 left outer join
...
SUM(purchaseid_ind) as purchased from crmanalytics.hit_data_agg
 where date_time between  DATE_ADD('${hiveconf:DATE_NOW}', -367) and DATE_ADD('${hiveconf:DATE_NOW}', -1) 
 ...
 drop table if exists ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model6_ext ;
 create external table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model6_ext 
 (customer_key bigint ,
...
insert overwrite table ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model6_ext 
...
pct_off_hit_ind_tot ,
purchased    from ${hiveconf:database}.${hiveconf:CURRENT_BRAND}_disc_sens_model5_tmp ;
