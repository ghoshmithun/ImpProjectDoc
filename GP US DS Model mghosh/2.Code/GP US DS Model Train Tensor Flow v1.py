...
with open('//10.8.8.51/lv0/Move to Box/Mithun/projects/7. DS_New Training/3.Documents/gp_ds_training_tfl_' + str(
        min(size_list)) + '_' + str(max(size_list)) + '_V1.txt', 'w',
          newline='') as fp:
...
for i in list(range(0, len(size_list))):
	...
	# create model 
        ...
        classifier.add(Dense(35, input_dim=35, init='uniform', activation='relu'))
        classifier.add(Dense(16, init='uniform', activation='relu'))
        classifier.add(Dense(8, init='uniform', activation='tanh'))
        ...
        # Fit the model
        classifier.fit(x=gp_data_train[:,1:], y=gp_data_train[:,0], nb_epoch=150, batch_size=100)
        
        
        # evaluate the model
        scores = classifier.evaluate(x=gp_data_train[:,1:], y=gp_data_train[:,0])
        ...
        for k in list(range(0,10)):
            ...
            with open('//10.8.8.51/lv0/Move to Box/Mithun/projects/7. DS_New Training/3.Documents/gp_ds_testing_tfl_' + str(min(size_list)) + '_' + str(max(size_list)) + '_V1.txt', 'a', newline='') as fp:
                a = csv.writer(fp, delimiter='|')
