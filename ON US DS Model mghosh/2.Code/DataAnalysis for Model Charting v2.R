...
var_plot <- function(var_names,Response=data_full2$Response,data=data_full2){ 
  require(ggplot2)
  require(dplyr)
  p<-list()
  ...
  pdf("C:/Users/mghosh/Box Sync/9.DS_New_Training_ON/3.Documents/Variable_Response_Plot.pdf", onefile = TRUE)
...
data_full <- read.table.ffdf(file="//10.8.8.51/lv0/Move to Box/Mithun/projects/7. DS_New Training/1. data/ON_disc_sens_Data_pull.txt",sep="|",header=F,VERBOSE = T)        

names(data_full) <- c('customer_key', 'flag_employee',
...
on_data <- data_full[sample(x = nrow(data_full),size = 0.3*nrow(data_full)),]

on_data$offline_last_txn_date[on_data$offline_last_txn_date==""]<-'2014-10-31'
on_data$offline_disc_last_txn_date[on_data$offline_disc_last_txn_date==""]<-'2014-10-31'

on_data$Time_Since_last_purchase <- as.numeric(as.Date("2015-11-01")-as.Date(on_data$offline_last_txn_date),units = "days")
on_data$Time_Since_last_disc_purchase <- as.numeric(as.Date('2015-11-01')-as.Date(on_data$offline_disc_last_txn_date),units = "days")

require(dplyr)

on_data$Response <- ifelse(on_data$resp_disc_percent < -0.35 , 1, 0)
on_data$Response[is.na(on_data$Response)] <- 0

var_names <- setdiff(colnames(on_data),c("customer_key","Response","acquisition_date","masterkey","online_last_txn_date","resp_disc_percent","offline_last_txn_date","offline_disc_last_txn_date","customerkey"))
on_data_var_types <- sapply(on_data[,var_names],class) 

on_data$card_status <- as.factor(on_data$card_status)

var_plot(var_names=var_names , Response=on_data$Response,data=on_data )
