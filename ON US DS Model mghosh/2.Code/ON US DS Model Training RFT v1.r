...
write.table(headers1, paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/8.ON_US_DS_Project/3.Documents/model/ON_US_DS_RFT_train_', min.sample.size,'_', max.sample.size, '.csv'), 
...
write.table(headers2, paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/8.ON_US_DS_Project/3.Documents/model/ON_US_DS_RFT_test_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
  ...
  for (s in 1:10)
  {
    ...
    for(j in 1:length(ntree.list))
    {
      ...
      for (ls in 1:length(nodesize.list))
      {
        ...
        rft.object <- paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/8.ON_US_DS_Project/Model_Objects/RandomForest/ON_US_DS_RFT_', sample.sizes[i],'_', s, '_', 
                             ntree.list[j], '_', nodesize.list[ls], '.RData')
        
        rft.model <- randomForest(y=as.factor(tra.on[,1]), x=tra.on[,c(-1)], ntree=100, 
                                  mtry=round(sqrt(ncol(tra.on[,c(-1)]))), replace=T,
        ...
        acc <- (rft.model$confusion[1,1] + rft.model$confusion[2,2])/nrow(tra.on)
        
        print('------------------------------------------')
        print(rft.model)
        print('------------------------------------------')
        
        prob_tra<- predict(rft.model, tra.on[,c(-1)], type="prob")
        ...
        write.table(cbind(sample.sizes[i], s, ntree.list[j], nodesize.list[ls], tpr, 
                    ...
                    paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/8.ON_US_DS_Project/3.Documents/model/ON_US_DS_RFT_train_', min.sample.size,'_', max.sample.size, '.csv'), 
        ...
        for (l in 1:10)
        {
          ...
          gc()
          prob_tst <- predict(rft.model,tst.on[,c(-1)],type="prob")
          ...
          write.table(cbind(sample.sizes[i], s, ntree.list[j], nodesize.list[ls], l,
                            roc.area.tst, concordance.tst),
                      paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/8.ON_US_DS_Project/3.Documents/model/ON_US_DS_RFT_test_', min.sample.size,'_', max.sample.size, '.csv'), 
                      append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)
