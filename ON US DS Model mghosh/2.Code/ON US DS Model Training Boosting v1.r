...
write.table(headers1, paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model/on_US_bst_training_', min.sample.size,'_', max.sample.size, '.csv'), 
...
write.table(headers2, paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model/on_US_bst_testing_', min.sample.size,'_', max.sample.size, '.csv'), 
            append=FALSE, sep=",",row.names=FALSE,col.names=FALSE)
 ...
 model_data_size <- round(0.15*total_obs)
  ...
  for (s in 1:5)
  {
    ...
    for (ls in 1:length(coeflearn.list))
    {
      ...
      roc.area <- auc(roc( prob_tra[,2],tra.bf[,1]))
      ...
      write.table(cbind(sample.sizes[i], s, coeflearn.list[ls], tpr, tnr, acc, cvacc, roc.area, concordance),
                  paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model/on_US_bst_training_', min.sample.size,'_', max.sample.size, '.csv'), 
      ...
      for (l in 1:10)
      {
        ...
        roc.area.tst <- auc(roc( prob_tst[,2], as.factor(tst.bf[,1])))
        ...
        write.table(cbind(sample.sizes[i], s, coeflearn.list[ls], l, roc.area.tst, concordance.tst),
                    paste0('//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model/on_US_bst_testing_', min.sample.size,'_', max.sample.size, '.csv'), 
                    append=TRUE, sep=",",row.names=FALSE,col.names=FALSE)
