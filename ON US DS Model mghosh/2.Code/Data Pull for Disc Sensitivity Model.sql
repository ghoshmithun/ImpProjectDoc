...
set BRAND = ON;
set database=mghosh_${hiveconf:BRAND};
...
--select count(1) from ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_offline_model1_tmp;
----  27257261
...
select a.*, h.on_net_sales_amt_12_mth, h.on_net_sales_amt_12_mth/b.BR_Net_Sales_Amt_12mth as on_br_sales_ratio, c.Num_Disc_Comm_responded , d.percent_disc_last_6_mth, d.percent_disc_last_12_mth ,
h.on_net_sales_amt_12_mth/e.GO_Net_Sales_Amt_12mth as on_go_net_sales_ratio , h.on_net_sales_amt_12_mth/f.BF_Net_Sales_Amt_12mth as on_bf_net_sales_ratio ,
h.on_net_sales_amt_12_mth/g.GP_Net_Sales_Amt_12mth as on_gp_net_sales_ratio
from  ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_model2_tmp a 
left outer join 
(select customer_key,sum(BR_Net_Sales_Amt_12mth)  as BR_Net_Sales_Amt_12mth from mghosh_ds.br_txn_last_12mth_tmp group by customer_key) b
on a.customer_key=b.customer_key
...
on a.customer_key=d.customer_key 
left outer join 
(select customer_key,sum(GO_Net_Sales_Amt_12mth)  as GO_Net_Sales_Amt_12mth from mghosh_ds.GO_txn_last_12mth_tmp group by customer_key) e 
on a.customer_key=e.customer_key
left outer join 
(select customer_key,sum(BF_Net_Sales_Amt_12mth)  as BF_Net_Sales_Amt_12mth from mghosh_ds.BF_txn_last_12mth_tmp group by customer_key) f 
...
left outer join
( select customer_key , sum(gross_sales_amt + discount_amt) as on_net_sales_amt_12_mth from ${hiveconf:database}.on_customer_txn_last_year group by customer_key) h 
...
on_net_sales_amt_12_mth double ,
on_br_sales_ratio double ,
num_disc_comm_responded bigint ,
percent_disc_last_6_mth double ,
percent_disc_last_12_mth double ,
on_go_net_sales_ratio double ,
on_bf_net_sales_ratio double ,
on_gp_net_sales_ratio double ,
card_status int ,
disc_ats double ,
non_disc_ats double ,
...
lines terminated by '\n' 
location '/user/mghosh/Disc_Sens_on'
...
bag_add_ind_tot  ,
purchased  ,
resp_disc_percent    from ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_model5_tmp a ;

 
hadoop fs -getmerge /user/mghosh/Disc_Sens_on/ /home/mghosh/Disc_Sens_fulldata/ON_disc_sens_Data_pull.txt
