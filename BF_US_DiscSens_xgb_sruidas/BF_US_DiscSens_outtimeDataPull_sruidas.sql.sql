...
SET DATE_NOW=2015-03-01;
set mapred.job.queue.name=default;
set mapred.job.priority=normal;
SET hive.cli.print.header=true;
SET DATEINDEX = 20150301;
set BRAND = BF;
...
drop table if exists sruidas.${hiveconf:BRAND}_customer_txn_resp_period ;
...
customer_key,
date_time
from  sruidas.${hiveconf:BRAND}_browse_data_20150301 a 
WHERE 
...
customer_key,
date_time
from (select * from  sruidas.${hiveconf:BRAND}_browse_data1_${hiveconf:DATEINDEX}
union all
select * from sruidas.${hiveconf:BRAND}_browse_data2_${hiveconf:DATEINDEX}
union all
select * from sruidas.${hiveconf:BRAND}_browse_data3_${hiveconf:DATEINDEX}
union all
select * from sruidas.${hiveconf:BRAND}_browse_data4_${hiveconf:DATEINDEX}
union all
select * from sruidas.${hiveconf:BRAND}_browse_data5_${hiveconf:DATEINDEX}
union all
select * from sruidas.${hiveconf:BRAND}_browse_data6_${hiveconf:DATEINDEX} )a 
WHERE 
date_time is not null AND 
unk_shop_id is not null AND
length(unk_shop_id) = 32 AND
...
SUM(purchaseid_ind) as purchased from sruidas.hit_data_agg_subset
 ...
 --5206883
  ...
  drop table if exists sruidas.${hiveconf:BRAND}_disc_sens_model6_ext_outtime ;
 create external table sruidas.${hiveconf:BRAND}_disc_sens_model6_ext_outtime 
 (customer_key bigint ,
...
non_disc_ats double ,
bf_go_net_sales_ratio double,
bf_on_sales_ratio double,
on_sales_rev_ratio_12mth double,
bf_br_net_sales_ratio double,
bf_gp_net_sales_ratio double,
...
lines terminated by '\n' 
location '/user/sruidas/brfs_discsen/${hiveconf:BRAND}_disc_sens_model6_ext_outtime'
TBLPROPERTIES ("serialization.null.format"=""); 


insert overwrite table sruidas.${hiveconf:BRAND}_disc_sens_model6_ext_outtime 
...
--hadoop fs -getmerge /user/sruidas/brfs_discsen/BF_disc_sens_model6_ext_outtime  /home/sruidas/brfs_discsen/bf_disc_sens_model6_outtime.txt
...
---This is how you can run Hive script from shell
---hive -hiveconf num=3 -e ' set num; select * from demodb.demo_table limit ${hiveconf:num}' 
