...
write.table(x = header,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model_report/bf_ds20_xgb_intime_score.txt",
            sep=",",col.names=F,row.names=F,append=F)
BF_data <-read.table.ffdf(file="//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/1.data/BF_data_full_transformed_disc20.txt",
                          sep="|",header = T,VERBOSE = TRUE,colClasses = c(rep("numeric",50)))
...
BF_data <- BF_data[,colnames(BF_data) %in% c('customer_key',"Response",model_predictor)]
bf_data <- subset(BF_data,select=c('customer_key','Response','ratio_rev_rewd_12mth',
                                           'num_days_on_books',
                                           'percent_disc_last_6_mth',
                                           'time_since_last_retail_purchase',
                                           'num_units_12mth',
...
write.table(x = score_file[,c(1,2,23,24)],file="//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model_report/bf_ds20_xgb_intime_score.txt",sep=",",col.names=F,row.names=F,append=T)
Score_Data <- read.table(file = "//10.8.8.51/lv0/Move to Box/Mithun/projects/12.BRFS_US_DS_Project/3.Documents/model_report/bf_ds20_xgb_intime_score.txt",header=T,sep=",",stringsAsFactors=FALSE)
