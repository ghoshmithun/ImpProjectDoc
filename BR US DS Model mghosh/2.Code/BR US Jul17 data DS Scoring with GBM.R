...
header <- cbind("Customer_key","MeanScore","MedianScore")

write.table(x = header,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Jul17/BR_US_DS_Score_Jul17.txt",sep=",",col.names=F,row.names=F,append=F)
...
require(R.utils)
#total_row_scoredata <- countLines("//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Jul17/BR_disc_sens_Data_pull_Jul17.txt")
#4475930
chunk_size <- 5000000
...
BR_data <- read.table(file = "//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Jul17/BR_disc_sens_Data_pull_Jul17.txt",
...
BR_data <- BR_data[,c('customer_key',model_predictor)]
...
names(BR_data_final) <- c('customer_key',model_predictor)
...
write.table(x = score_file[,c(1,22,23)],file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Jul17/BR_US_DS_Score_Jul17.txt",sep=",",col.names=F,row.names=F,append=T)
print("##################################################################")
print(paste("scoring for data chunk", loop , "complete"))
print("##################################################################")
}

score_file_final <- as.data.frame(score_file[,c(1,22,23)])

names(score_file_final) <- c("Customer_Key","MeanScore","MedianScore")

score_file_final$Customer_Key <- as.character(score_file_final$Customer_Key)

write.table(x = score_file_final,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Jul17/BR_US_DS_Score_Jul17_v2.txt",sep=",",col.names=F,row.names=F,append=T)
