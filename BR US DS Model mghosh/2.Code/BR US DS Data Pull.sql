...
SET DATE_NOW=2016-01-01;
set mapred.job.queue.name=aa;
set mapred.job.priority=VERY_HIGH;
SET hive.cli.print.header=true;
SET DATEINDEX = 20160101;
set BRAND = BR;
set database=mghosh_ds_${hiveconf:BRAND};
...
create table ${hiveconf:database}.GP_txn_last_12mth_tmp as s
...
on a.customer_key=k.customer_key  ;
 ...
 --Browse variables.. BROWSE DATA is for all brand no need to pull separately for ON
...
--SET DATE_NOW=2016-01-01;
--SET DATEINDEX = 20160101;
SET DATE_NOW=2015-10-01;
SET DATEINDEX =20151001;
SET DATE_NOW=2015-07-01;
SET DATEINDEX =20150701;
SET DATE_NOW=2015-04-01;
SET DATEINDEX = 20150401;

drop table if exists ${hiveconf:database}.${hiveconf:BRAND}_browse_data_${hiveconf:DATEINDEX} ;
create table ${hiveconf:database}.${hiveconf:BRAND}_browse_data_${hiveconf:DATEINDEX} as
...
WHERE date_time between  DATE_ADD('${hiveconf:DATE_NOW}', - 91 ) and DATE_ADD('${hiveconf:DATE_NOW}', -1) 
...
customer_key,
date_time
from (select * from  ${hiveconf:database}.${hiveconf:BRAND}_browse_data_20150401
union all
select * from ${hiveconf:database}.${hiveconf:BRAND}_browse_data_20150701
union all
select * from ${hiveconf:database}.${hiveconf:BRAND}_browse_data_20151001
union all
select * from ${hiveconf:database}.${hiveconf:BRAND}_browse_data_20160101 ) a 
WHERE 
...
select a.customer_key, b.addr_zip_code 
from mds_new.ods_corp_cust_t a inner join 
(select household_key,addr_zip_code  from mds_new.ods_household_t where ctry_cd = 'US' ) b
on a.household_key=b.household_key;
 ...
 select  t1.*,t4.addr_zip_code, t2.*, t3.Resp_Disc_Percent
...
SUM(purchaseid_ind) as purchased from ${hiveconf:database}.hit_data_agg_subset
 ...
 left outer join
 (select customer_key, coalesce(sum(discount_amt),0) / coalesce(sum(gross_sales_amt) , 0 ) as Resp_Disc_Percent from ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_resp_period 
 group by customer_key) t3
 on t1.customer_key=t3.customer_key
 left outer join
 (select customer_key, addr_zip_code from ${hiveconf:database}.cust_household_list) t4
...
lines terminated by '\n' 
location '/user/mghosh/Disc_Sens_BR'
...
bag_add_ind_tot    ,
purchased    ,
resp_disc_percent       from ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_model5_tmp  ;

 
hadoop fs -getmerge /user/mghosh/Disc_Sens_BR/ /home/mghosh/Disc_Sens_fulldata/BR_disc_sens_Data_pull.txt
...
drop table if exists ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_Resp_Time;
create external table ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_Resp_Time 
(customer_key bigint, Gross_Sales_Amt double, Net_Sales_Amt double , Resp_Disc_Percent double ,item_qty int)
row format delimited 
fields terminated by '|' 
lines terminated by '\n' 
location '/user/mghosh/Disc_Sens_BR_resp'
TBLPROPERTIES ("serialization.null.format"="");



insert overwrite table ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_Resp_Time
select customer_key, sum(coalesce(gross_sales_amt,0))  as Gross_Sales_Amt, sum(coalesce(gross_sales_amt,0) + coalesce(discount_amt,0)) as Net_Sales_Amt , coalesce(sum(discount_amt),0) / coalesce(sum(gross_sales_amt) , 0 ) as Resp_Disc_Percent ,sum(coalesce(item_qty,0)) as item_qty from ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_resp_period group by customer_key ;


hadoop fs -getmerge /user/mghosh/Disc_Sens_BR_resp/ /home/mghosh/Disc_Sens_fulldata/BR_disc_sens_Data_pull_response.txt
...
--- New data Pull for Geo Specific Map Creation
...
drop table if exists ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data ;
create table ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data as
select a.customer_key,	a.percent_disc_last_12_mth,	a.br_net_sales_amt_12_mth,	a.flag_employee, a.addr_zip_code,a.resp_disc_percent,a.browse_hit_ind_tot,a.purchased, b.br_Online_Sales12_mth,c.br_Offline_Sales_12mth 
from ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_model5_tmp a 
left outer join
(select customer_key , sum(gross_sales_amt + discount_amt) as br_Online_Sales12_mth from ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_last_year where order_status='O' group by customer_key ) b 
on a.customer_key=b.customer_key
left outer join
(select customer_key , sum(gross_sales_amt + discount_amt) as br_Offline_Sales_12mth from ${hiveconf:database}.${hiveconf:BRAND}_customer_txn_last_year  where order_status='R' group by customer_key) c
on a.customer_key=c.customer_key;

drop table if exists ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data_ext;
create external table ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data_ext 
(customer_key bigint,
percent_disc_last_12_mth double,
br_net_sales_amt_12_mth double,
flag_employee string,
...
browse_hit_ind_tot bigint,
purchased bigint,
br_online_sales12_mth double,
br_offline_sales_12mth double)
row format delimited 
fields terminated by '|' 
lines terminated by '\n' 
location '/user/mghosh/Disc_Sens_BR'
...
insert overwrite table ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data_ext 
select distinct customer_key,
percent_disc_last_12_mth,
br_net_sales_amt_12_mth,
flag_employee,
...
br_offline_sales_12mth from ${hiveconf:database}.${hiveconf:BRAND}_disc_sens_geo_data;

hadoop fs -getmerge /user/mghosh/Disc_Sens_BR/ /home/mghosh/Disc_Sens_fulldata/BR_disc_sens_geo_data.txt