...
write.table(x = header,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score.txt",sep=",",col.names=F,row.names=F,append=F)
...
require(R.utils)
total_row_scoredata <- countLines("//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_disc_sens_Data_pull_Feb17.txt")
#4558799
chunk_size <- 5000000
...
BR_data <- read.table(file = "//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_disc_sens_Data_pull_Feb17.txt",
...
names(BR_data) <- c('customer_key' ,'flag_employee' ,'offline_last_txn_date' ,'online_last_txn_date' ,'offline_disc_last_txn_date' ,'total_plcc_cards' ,'acquisition_date' ,
                    ...
                    'num_units_6mth' ,'num_txn_6mth' ,'num_units_12mth' ,'num_txn_12mth' ,'ratio_rev_rewd_12mth' ,'ratio_rev_wo_rewd_12mth' ,'num_em_campaign' ,'per_elec_comm' ,
                    'on_sales_item_rev_12mth' ,'on_sales_rev_ratio_12mth' ,'on_sale_item_qty_12mth' ,'customerkey' ,'masterkey' ,'mobile_ind_tot' ,'searchdex_ind_tot' ,
                    'gp_hit_ind_tot' ,'br_hit_ind_tot' ,'on_hit_ind_tot' ,'at_hit_ind_tot' ,'factory_hit_ind_tot' ,'sale_hit_ind_tot' ,'markdown_hit_ind_tot' ,
                    'clearance_hit_ind_tot' ,'pct_off_hit_ind_tot' ,'browse_hit_ind_tot' ,'home_hit_ind_tot' ,'bag_add_ind_tot' ,'purchased' )

model_predictor <- c('customer_key','percent_disc_last_12_mth', 'num_days_on_books','per_elec_comm','num_em_campaign','num_units_12mth','disc_ats','time_since_last_retail_purchase',
...
BR_data <- BR_data[,model_predictor]

model_predictorx <- model_predictor[-1]
BR_data_capped <- do.call("cbind.data.frame",lapply(seq_along(model_predictorx),cap_value))

# Test the accuracy by 
#head(cbind(BR_data[,28],BR_data_capped[,27]))
#tail(cbind(BR_data[,28],BR_data_capped[,27]))

BR_data_final <- cbind.data.frame(BR_data$customer_key,BR_data_capped)

names(BR_data_final) <- model_predictor
...
score_file <- matrix(0,nrow=nrow(BR_data_final),ncol=23)
for (j in 1:20){
  require(gbm)
...
write.table(x = score_file,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score.txt",sep=",",col.names=F,row.names=F,append=T)
print("##################################################################")
...
require(ffbase)
require(ff)
Score_Data <- read.table.ffdf(file = "//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score.txt",header=TRUE,sep=",",VERBOSE = T,colClasses=c(rep("numeric",23)))

Score_Data_submission <- Score_Data[,c(1,23)]
write.table(x = Score_Data_submission,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score_final.txt",sep=",",col.names=F,row.names=F,append=F)
Score_Data_submission$Customer_key <- as.character(Score_Data_submission$Customer_key)
write.table(x = Score_Data_submission,file="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score_final_v2.txt",sep=",",col.names=F,row.names=F,append=F)

require(haven)

write_sas(data=Score_Data_submission, path="//10.8.8.51/lv0/Move to Box/Mithun/projects/9.BR_US_DS_Project/1.data/Feb17/BR_US_DS_Score_final_sas.sas7bdat")
